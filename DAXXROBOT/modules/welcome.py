...
from DAXXROBOT import DEV_USERS
from Mikobot import DEV_USERS as SUDO
from DAXXROBOT import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, app, dispatcher, function
from DAXXROBOT.plugins.helper_funcs.chat_status import check_admin, is_user_ban_protected
from DAXXROBOT.plugins.helper_funcs.misc import build_keyboard, revert_buttons
from DAXXROBOT.plugins.helper_funcs.msg_types import get_welcome_type
from DAXXROBOT.plugins.helper_funcs.string_handling import escape_invalid_curly_brackets
from DAXXROBOT.plugins.log_channel import loggable
from DAXXROBOT.utils.can_restrict import can_restrict
    ...
    if user.id in SUDO:
        ...
        try:
            ...
            temp.MELCOW[f"welcome-{chat_id}"] = await client.send_photo(
                member.chat.id,
                photo=welcomeimg,
                caption=f"**𝗛𝗲𝘆 💖 {mention},  𝗜 𝗔𝗺 𝗡𝗮𝗿𝘂𝘁𝗼 𝗫 𝗥𝗼𝗯𝗼𝘁 𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝗧𝗼 {member.chat.title} 𝗚𝗿𝗼𝘂𝗽.**\n\n**➖➖➖➖➖➖➖➖➖➖➖➖**\n**𝗡𝗔𝗠𝗘 : {first_name}**\n**𝗜𝗗 : {user_id}**\n**𝗗𝗔𝗧𝗘 𝗝𝗢𝗜𝗡𝗘𝗗 : {joined_date}**",
            )
